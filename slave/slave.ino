...
// ═══════════════════════════════════════════════════════
// TRAVEL CONSTANTS (45,000 steps = full travel)
// ═══════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════
// HOMING — 3-minute timeout per motor
// If a sensor isn't hit within timeout, that motor is
// force-zeroed and marked homed to avoid system lockup
// ═══════════════════════════════════════════════════════
void homeAllMotors() {
  ...
  while (!(h[0] && h[1] && h[2] && h[3] && h[4])) {
    ...
    for (int i = 0; i < 5; i++) {
      ...
      if (digitalRead(homePins[i]) == LOW) {
        ...
        h[i] = true;
        Serial.printf("  ✓ M%d homed (sensor)\n", i + 1);
      }
      // Timeout — force zero and continue
...
void patternTwo() {
  // Wave Cascade M1 → M5, simultaneous return
...
void patternThree() {
  // Alternating Pulse — odd full / even quarter, then swap
  for (int r = 0; r < 3; r++) {
...
void patternFour() {
  // Breathing — arch deepest at center, then inverted
...
void setup() {
  ...
  // Re-register peer locked to found channel
...
void loop() {
  if (activePattern > 0) {
    ...
    activePattern = 0; // Return to idle
  }
  yield();
}